...
def mark_keywords(json_path, keywords):
    scr = load_books_from_json_file(path=json_path)
    scr.mark_keywords(keywords=keywords)

    for book in scr.books.books:
        print(f"{book.title}")


def scrap_and_mark(url_params, html_path, page_title, keywords, open_in_browser=False):
    scr = get_books(url=url_params.get_full_url(), url_params={},
                    num_of_pages=1, start_from_page=1, save_as_json=False)
...
if __name__ == "__main__":
    try:
        ...
        # mark_keywords("scrapped_pages/django.json",
        #               keywords=["python", "django", "websites"])

        url_params = ScrapperUrlParams(base_url="https://1lib.pl", search_term="css", year_from=2022,
                                       year_to=2022, languages=["english"], order="date")

        scrap_and_mark(url_params=url_params, html_path="scrapped_pages/css_with_keywords.html",
                       page_title="CSS books", keywords=["html", "layout", "tailwind"])
